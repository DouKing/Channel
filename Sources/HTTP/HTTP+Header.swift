...
extension HTTP.Header {
    ...
    public struct Value: Hashable, Equatable, RawRepresentable, @unchecked Sendable {
        public let rawValue: String
...
extension HTTP.Header.Value {
    ...
    public static var formUrlEncoded = HTTP.Header.Value("application/x-www-form-urlencoded; charset=utf-8")
    /// application/json
    public static var json = HTTP.Header.Value("application/json")
}
...
extension HTTP.Header {
    ...
    ///
    /// Channel offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// Channel offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// Channel provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All Channel `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not
...
extension HTTP.Header {
    /// Returns Channel's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns Channel's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns Channel's default `User-Agent` header.
    ///
