    ...
    open class MultipartFormData {
        ...
        enum BoundaryGenerator {
            ...
            static func randomBoundary() -> String {
                ...
                return String(format: "channel.boundary.%08x%08x", first, second)
            }
        ...
        class BodyPart {
            ...
            init(headers: [HTTP.Header], bodyStream: InputStream, bodyContentLength: UInt64) {
        ...
        private var bodyPartError: HTTP.Error?
...
extension HTTP.MultipartFormData {
    ...
    ///   - stream:  `InputStream` to encode into the instance.
    ///   - length:  Length, in bytes, of the stream.
    ///   - headers: `[HTTP.Header]` for the body part.
    public func append(_ stream: InputStream, withLength length: UInt64, headers: [HTTP.Header]) {
...
extension HTTP.MultipartFormData {
    ...
    /// - Throws:  An `HTTPError` if encoding encounters an error.
    ...
    public func writeEncodedData(to fileURL: URL) throws {
        ...
        if fileManager.fileExists(atPath: fileURL.path) {
            throw HTTP.Error.multipartEncodingFailed(reason: .outputStreamFileAlreadyExists(at: fileURL))
        } else if !fileURL.isFileURL {
            throw HTTP.Error.multipartEncodingFailed(reason: .outputStreamURLInvalid(url: fileURL))
        }

        guard let outputStream = OutputStream(url: fileURL, append: false) else {
            throw HTTP.Error.multipartEncodingFailed(reason: .outputStreamCreationFailed(for: fileURL))
        }
...
extension HTTP.MultipartFormData {
    ...
    private func encodeBodyStream(for bodyPart: BodyPart) throws -> Data {
        ...
        while inputStream.hasBytesAvailable {
            ...
            if let error = inputStream.streamError {
                throw HTTP.Error.multipartEncodingFailed(reason: .inputStreamReadFailed(error: error))
            }
        ...
        guard UInt64(encoded.count) == bodyPart.bodyContentLength else {
            let error = HTTP.Error.UnexpectedInputStreamLength(bytesExpected: bodyPart.bodyContentLength,
    ...
    private func writeBodyStream(for bodyPart: BodyPart, to outputStream: OutputStream) throws {
        ...
        while inputStream.hasBytesAvailable && bytesLeftToRead > 0 {
            ...
            if let streamError = inputStream.streamError {
                throw HTTP.Error.multipartEncodingFailed(reason: .inputStreamReadFailed(error: streamError))
            }
    ...
    private func write(_ buffer: inout [UInt8], to outputStream: OutputStream) throws {
        ...
        while bytesToWrite > 0, outputStream.hasSpaceAvailable {
            ...
            if let error = outputStream.streamError {
                throw HTTP.Error.multipartEncodingFailed(reason: .outputStreamWriteFailed(error: error))
            }
    ...
    private func contentHeaders(withName name: String, fileName: String? = nil, mimeType: String? = nil) -> [HTTP.Header] {
        var disposition = "form-data; name=\"\(name)\""
        if let fileName { disposition += "; filename=\"\(fileName)\"" }

        var headers: [HTTP.Header] = [.contentDisposition(disposition)]
        if let mimeType { headers.append(.contentType(mimeType)) }
    ...
    private func setBodyPartError(withReason reason: HTTP.Error.MultipartEncodingFailureReason) {
        guard bodyPartError == nil else { return }
        bodyPartError = HTTP.Error.multipartEncodingFailed(reason: reason)
    }
}
