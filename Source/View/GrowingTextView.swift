...
open class GrowingTextView: UITextView {
	...
	override open func layoutSubviews() {
		...
		if (heightConstraint == nil) {
			heightConstraint = NSLayoutConstraint(
                item: self, attribute: .height, relatedBy: .equal, toItem: nil,
	...
	private func scrollToCorrectPosition() {
		if self.isFirstResponder {
            self.scrollRangeToVisible(.init(location: -1, length: 0)) // Scroll to bottom
		} else {
            self.scrollRangeToVisible(.init(location: 0, length: 0)) // Scroll to top
		}
	}
